...
module.exports = ({ db, middlewares, configuration, regions, logger }) => {
    ...
    router.get('/backoffice/financeur/stats', checkAuth, checkProfile('financeur'), tryAndCatch(async (req, res) => {
        ...
        return res.json({
            ...
            avis: Object.keys(avis[0]).reduce((acc, key) => {

                //Mongo can not yet round with 2 digits
                let roundedValue = round(avis[0][key]);

                //Wrap all notes under a parent property (can be done with $project)
                if (key.indexOf('_') !== -1) {
