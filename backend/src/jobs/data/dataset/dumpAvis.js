...
module.exports = async (db, dumpFile) => {
    ...
    let [published, rejected, reported, toModerate] = await Promise.all([
        ...
        db.collection('comment').find({ published: true }).project(projection).sort({ date: -1 }).limit(5).toArray(),

        db.collection('comment').find({ rejected: true }).project(projection).sort({ date: -1 }).limit(5).toArray(),

        db.collection('comment').find({ reported: true }).project(projection).sort({ date: -1 }).limit(5).toArray(),
