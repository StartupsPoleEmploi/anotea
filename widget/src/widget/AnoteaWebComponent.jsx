...
class WidgetAnotea extends HTMLElement {
    connectedCallback() {
        ...
        if(layout === 'tiny') {
            ReactDOM.render(<AnoteaTinyWidget layout={layout} width={this.getAttribute('width')} niveau={this.getAttribute('niveau')} siret={this.getAttribute('siret')} numeroAction={this.getAttribute('numeroAction')} />, shadowRoot);
        } else {
